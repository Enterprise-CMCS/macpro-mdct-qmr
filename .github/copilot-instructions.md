...
### Never commit code directly to the branch without confirming with the PR author
...
### Label findings by type and severity

Prefix every comment with a label so reviewers can quickly triage. Labels fall into three severity tiers:

**Non-Blocking** — never require changes before merging:

- **Praise:** Something done well; positive reinforcement.
- **Nit:** Unimportant, opinion-based note; purely stylistic.
- **Comment:** Tangential observation not directly tied to the diff.
- **Thought / Idea:** Ideas for future improvements, not actionable now.
- **Suggestion:** An alternative approach or educational note.

**Neutral** — may or may not require action depending on context:

- **Convention:** Code doesn't meet team standards (may need linting or a follow-up ticket).
- **Question:** Something confusing that needs clarification before the reviewer can assess it.
- **Typo:** A typo or trivially fixable mistake.

**Blocking** — must be resolved or explicitly accepted before merging:

- **Request:** Something that should be changed, added, or removed.
- **Blocking:** A must-fix — merging as-is would introduce a bug, break behavior, or violate a hard requirement.

It is ultimately up to the human reviewers to decide whether a finding blocks the PR. When in doubt, prefer a lower severity and let the team escalate.
