...
export async function auditUploadsLambda(event, _context) {
...
async function emptyWorkdir(workdir) {
...
async function auditBucket(s3Client, clamAV, bucketName) {
  ...
  for (const chunk of chunks) {
    ...
    const downloadRes = await s3Client.downloadAllFiles(
      chunk,
      bucketName,
      "/tmp/download"
    );
