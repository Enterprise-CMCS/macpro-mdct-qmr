...
describe("Test DateRange", () => {
  ...
  test("Check that a year with incorrect format will throw an error", async () => {
    const screen = renderWithHookForm(<DateRange name="testComponent" />);

    const monthLabel = screen.getAllByLabelText(/month/i)[0];
    const yearLabel = screen.getAllByLabelText(/year/i)[0];
    await userEvent.type(monthLabel, "01");
    await userEvent.type(yearLabel, "202");
  ...
  test("Check that error disapears when user inputs correct date", async () => {
    const screen = renderWithHookForm(<DateRange name="testComponent" />);

    const monthLabel = screen.getAllByLabelText(/month/i)[0];
    const yearLabel = screen.getAllByLabelText(/year/i)[0];
    await userEvent.type(monthLabel, "01");
    await userEvent.type(yearLabel, "2022");
    expect(
      screen.queryByText("Please enter start date year in YYYY-format")
    ).not.toBeInTheDocument();
  });

  test("Check that error appears when user inputs a start date after the end date", async () => {
    const screen = renderWithHookForm(<DateRange name="testComponent" />);
    ...
    await userEvent.type(startMonthLabel, "01");
    await userEvent.type(yearLabel, "2023");
    await userEvent.type(endMonthLabel, "02");
    await userEvent.type(endYearLabel, "2022");
    await waitFor(() => {
      expect(
  ...
  test("Check that error appears when start date is a date in the future", async () => {
    const screen = renderWithHookForm(<DateRange name="testComponent" />);
    const startMonthLabel = screen.getAllByLabelText(/month/i)[0];
    const yearLabel = screen.getAllByLabelText(/year/i)[0];
    await userEvent.type(startMonthLabel, "01");
    await userEvent.type(yearLabel, "2080");
    expect(
      screen.queryByText("Start date cannot be a future date")
    ).toBeInTheDocument();
  });

  test("Check that error appears when end date is a date in the future", async () => {
    const screen = renderWithHookForm(<DateRange name="testComponent" />);

    const endMonthLabel = screen.getAllByLabelText(/month/i)[1];
    const endYearLabel = screen.getAllByLabelText(/year/i)[1];
    await userEvent.type(endMonthLabel, "02");
    await userEvent.type(endYearLabel, "2080");
    expect(
