...
describe("Measure: BCS-AD", () => {
  ...
  it("if only admin data cannot override, if anything else, rate is editable", () => {
    ...
    cy.get('[data-cy="PerformanceMeasure.rates.xacc8a.0.rate"]').should(
      "have.attr",
      "readonly"
    );
  ...
  it("Rates calculate correctly", () => {
    ...
    cy.get('[data-cy="PerformanceMeasure.rates.xacc8a.0.numerator"]').type("5");
    cy.get('[data-cy="PerformanceMeasure.rates.xacc8a.0.denominator"]').type(
      "6"
    );
    cy.get('[data-cy="PerformanceMeasure.rates.xacc8a.0.rate"]').should(
      "have.value",
    ...
    cy.get('[data-cy="DataSource2"] > .chakra-checkbox__control').click();
    cy.get('[data-cy="PerformanceMeasure.rates.xacc8a.0.numerator"]').clear();
    cy.get('[data-cy="PerformanceMeasure.rates.xacc8a.0.numerator"]').type("0");
    cy.get('[data-cy="PerformanceMeasure.rates.xacc8a.0.rate"]').clear();
    cy.get('[data-cy="PerformanceMeasure.rates.xacc8a.0.rate"]').type("20");
