...
describe("Measure: SSD-AD", () => {
  ...
  it("displays the correct calculated rate amount in total", () => {
    ...
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.numerator"]').type(
      "123"
    );
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.denominator"]').type(
      "456"
    );
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.rate"]').should(
      "have.value",
      "27.0"
    );
  });
  ...
  it("rounds auto-calculated rate up or down as expected", () => {
    // 3/9*100 = 3.333... -> 33.3
    cy.get('[data-cy="MeasurementSpecification0"]').click();
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.numerator"]').type("3");
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.denominator"]').type(
      "9"
    );
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.rate"]').should(
    ...
    // 6/9*100 = 66.666... -> 66.7
    cy.get('[data-cy="MeasurementSpecification0"]').click();
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.numerator"]').clear();
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.numerator"]').type("6");
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.denominator"]').clear();
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.denominator"]').type(
      "9"
    );
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.rate"]').should(
  ...
  it("displays a warning if a user entered a rate with N = 0, D > 0, and R > 0", () => {
    ...
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.numerator"]').clear();
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.numerator"]').type("0");
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.denominator"]').clear();
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.denominator"]').type(
      "456"
    );
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.rate"]').clear();
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.rate"]').type("10");
    cy.get('[data-cy="DataSource1"]').click();
  ...
  it("displays a warning if a user entered a rate with N > 0, D > 0, and R = 0", () => {
    ...
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.rate"]').clear();
    cy.get('[data-cy="PerformanceMeasure.rates.IJmKJZ.0.rate"]').type("0.0");
    cy.get('[data-cy="DataSource1"]').click();
    cy.get("#DataSource1-checkbox").uncheck();
    cy.get('[data-cy="Validate Measure"]').click();
    cy.get(
